...
class Config:
    ...
    ) or "sqlite:///" + os.path.join(basedir, "instance", "flaskdb.sqlite")
    SQLALCHEMY_TRACK_MODIFICATIONS = False

    # Seguridad
    SECRET_KEY = (
        os.getenv("SECRET_KEY")
        or "a2e4c8b7f1d9e3a6c5b8d7f2e1c4a9b6e7d3c2b1f8a6e5d4c3b2a1f9e8d7c6b5"
    )
    ...
    UPLOAD_FOLDER = os.getenv(
        "UPLOAD_FOLDER", os.path.join(basedir, "app", "static", "product_images")
    )
    ...
    SESSION_FILE_DIR = os.getenv(
        "SESSION_FILE_DIR", os.path.join(basedir, "instance", "sessions")
    )
